...
State-toggle actions such as bury, resurface, submerge, and similar specials should also be modeled as action segments rather than as passive flags. Underling and Cyber Underling do not merely “have a buried boolean”; they switch into a different movement/vision/defense mode, and resurface grants an attack bonus on the attack that follows. For buried Underling-family units specifically, the buried-turn action menu is effectively `move while buried` or `resurface` or `resurface+attack`. That last option is a **single indivisible action window**: once the unit resurfaces for attack, no other friendly unit may act before that attack is either taken or the activation is cancelled. In other words, a bot-ready state model must represent both persistent state (`buried`, `submerged`, cooldowns, infection, disabled) and the **activation template** that determines which sequences are legal for the currently selected unit.
...
That flow is directly consistent with the official rules page, the official gang-up description, official AP displays on unit pages, and the official calculator’s p-value framing. For Underling and Cyber Underling, add the published `+4` resurface bonus to the same attack as resurfacing; it stacks additively with terrain attack bonus and gang-up. citeturn43search5turn33view0turn24search2turn24search0turn12view2turn18view3
...
The official unit pages add a second layer of numbers for buried and submerged states. Underling has underground mobility 7, underground vision 2, and a `+4` resurface bonus; Cyber Underling has underground mobility 6, underground vision 2, defense 6 underground, and also `+4` resurface. Engine-wise, that should be interpreted narrowly: buried Underling-family units cannot attack while still buried, but if they resurface and attack immediately as part of the same action, that attack gets `+4` in addition to terrain and gang-up. Kraken has submerged mobility 10, submerged vision 3, submerged defense 12, underwater range 1, and `+3` resurface; Submarine and Skimmer have underwater ranges `1-2`, submerged defenses `8` and `9`, and all three submerged sea attackers publish `Attack from Underwater penalty -2` on their own unit pages. That field is attacker-side, not target-side: the effective hidden-mode attack against a submerged target is `Aquatic attack + Attack from Underwater penalty`, so Submarine attacks submerged targets at `8 + (-2) = 6`, Kraken at `6 + (-2) = 4`, and Skimmer at `9 + (-2) = 7`. Surface-mode penalties are published separately as `Attack Underwater units penalty` and are also attacker-side; the effective surface attack against a submerged target is `Aquatic attack + Attack Underwater units penalty`. The exact public per-unit values are: `-100` for Marine, Mecha II, Underling, Infected Marine, Mecha, and Cyber Underling; omitted entirely on the official page for Engineer, Infector, and Assimilator because they have no attack; `-8` for Marauder, Bopper, Tank, Battery, Borfly, Pinzer, Wyrm, Speeder, Guardian, Eclipse, Plasma Tank, and Walker; `-5` for Helicopter, Swarmer, and Garuda; `-1` for Fuze, Salamander, and Mantisse; and `-9` for Destroyer, Leviathan, and Hydronaut. Example conversions are Destroyer `16 + (-9) = 7`, Salamander `5 + (-1) = 4`, and Marine `2 + (-100)` which is effectively "cannot attack submerged targets." A negative derived value such as Marauder `4 + (-8) = -4` should be preserved as numeric combat input, not collapsed into "cannot target"; only `-100` or a missing published field should be treated as a hard unsupported attack mode. This distinction matters for a bot dictionary because the official pages do not publish a separate surface-versus-submerged penalty for Submarine, Kraken, or Skimmer; they only publish the submerged attack-mode penalty. citeturn6view0turn6view1turn6view2turn6view3turn6view4turn7view1turn7view2turn8view1turn8view2turn8view3turn8view4turn8view6turn8view7turn8view8turn8view9turn9view0turn9view1turn1search0turn1search7
